...
# ---------------------------------------------------------------------------
# Stationary Wavelet Transform (SWT) — detail-preserving variant
# ---------------------------------------------------------------------------
...
class SWT3d(nn.Module):
    ...
    Same filter bank as DWT3d but stride=1, so output retains full spatial
    resolution. Supports dilation (à trous algorithm) for multi-level use:
    dilation=2^(level-1) captures structure at progressively wider scales
    without ever downsampling.

        Input:  [B, 1, D, H, W]
        Output: [B, 8, D, H, W]   ← same size at every level

    Parameters
    ----------
    wavelet : str  — one of 'haar', 'db2', 'sym4'
    """

    def __init__(self, wavelet: str = "haar"):
        super().__init__()
        if wavelet not in _WAVELET_FILTERS:
            raise ValueError(f"Unknown wavelet '{wavelet}'. Available: {list(_WAVELET_FILTERS)}")
        lo_1d, hi_1d = _WAVELET_FILTERS[wavelet]
        L = torch.tensor(lo_1d, dtype=torch.float32)
        H = torch.tensor(hi_1d, dtype=torch.float32)
    ...
    def forward(self, x: torch.Tensor, dilation: int = 1) -> torch.Tensor:
        # Causal padding (k-1)*dilation keeps output size = input size
...
class WaveletPatchEmbedSWT(nn.Module):
    ...
    Applies SWT at `levels` scales using dilation=2^(l-1). Every level stays
    at full spatial resolution — no detail is ever lost to downsampling.

    Dilation schedule:
        level 1 : dilation=1  — fine local edges
        level 2 : dilation=2  — medium-scale structure
        level 3 : dilation=4  — wider lesion context

    Detail subbands (channels 1-7) are rectified (abs) before SE so edge
    energy — not cancelling signed values — drives channel recalibration.

    Pipeline (levels=2 example):
        [B, 1, D, H, W]
        → SWT(dil=1) → |detail| → SE₁ → [B, 8,  D, H, W]
        → SWT(dil=2) → |detail| → SE₂ → [B, 8,  D, H, W]
        → cat                           → [B, 16, D, H, W]
        → Conv3d(16→embed_dim, k=2, s=2)→ [B, embed_dim, D/2, H/2, W/2]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.levels):
            ...
            approx = sub[:, :1]
            detail = sub[:, 1:].abs()              # edge energy, not signed
